...
My hypothesis is that provinces' geographic characteristics (terrain ruggedness, purely inland or near shore, adjacency to recently conflicted areas), could have influenced strategies (use of cavalries) and outcomes (number of casualties).
...
My study is cross-sectional, as it analyzes data collected from a specific point in time (1467 - 1600) without manipulating the environment, and does not have any intervention or treatment.
I assume this is typical in the study of history, when we are dealing with pre-modern historical data.
...
From least to most, where did the battles take place?
```{r}
plot_provinces <- warringstate |>
  ...
  labs(x = "Province", y = "Count of Battles", fill = "Geography") +
...
```
Analysis:
The positive coefficient (7.029) suggests a baseline presence of cavalry in battles, which is statistically significant (p = 0.006), but not meaningful or interesting in reality.
The negative coefficient -0.055 of variable `adjacent_battle` suggests that, holding constant the time (year) and the number of adjacent provinces (neighbors), the existence of a recent battle in adjacent provinces is associated to a slight decrease in the likelihood of cavalry use in a current battel. However, this is not statistically significant (p = 0.745), meaning that we can't confidently infer this correlation.
