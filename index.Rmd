...
#plot the proportion of battles in inland provinces and shore provinces over years
landshore_prop <- warringstate |>
  mutate(shore = if_else(geography == "shore", 1, 0)) |>
  filter(!is.na(shore)) |>
  group_by(year, shore) |>
  summarize(count = n(), .groups = 'drop') |>
  ggplot(aes(x = year, y = count/n(warringstate))) +
  geom_point() +
  geom_smooth(na.rm = FALSE) +
  facet_wrap(~ shore, labeller = as_labeller(c('0' = 'prop of battles in inland provinces', '1' = 'prop of battles in shore provinces')))
