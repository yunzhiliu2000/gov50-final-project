...
```{r}
#Defining Inland and Near-shore Provinces
...
battledata <- battledata |>
  mutate(forcesA_recode = if_else(forcesA == "1,200~2,000",
                                  "1600", forcesA_recode),
         forcesA_recode = if_else(forcesA == "1,400~3,000",
                                  "2200", forcesA_recode))
```

```{r}
lm(victoryA ~ forcesA_recode + forcesB_recode + casualtiesA + casualtiesB)
```
...
```{r}
cavalry_terrain <- 
  lm(cavalry ~ terrain_ruggedness + neighbors + road_battle + area + year, data = warringstate)
summary(cavalry_terrain)
```
```{r}
modelsummary::modelsummary(cavalry_terrain,
